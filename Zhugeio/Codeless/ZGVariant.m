...
- (BOOL)isEqualToVariant:(ZGVariant *)variant {
...
- (BOOL)isEqual:(id)object {
    ...
    if (![object isKindOfClass:[ZGVariant class]]) {
        return NO;
    }

    return [self isEqualToVariant:(ZGVariant *)object];
}
...
+ (MPVariantAction *)actionWithJSONObject:(NSDictionary *)object
{
    ...
    if (!path) {
        ZGLogDebug(@"invalid action path: %@", object[@"path"]);
        return nil;
    }

    SEL selector = NSSelectorFromString(object[@"selector"]);
    if (selector == (SEL)0) {
        ZGLogDebug(@"invalid action selector: %@", object[@"selector"]);
        return nil;
    }

    NSArray *args = object[@"args"];
    if (![args isKindOfClass:[NSArray class]]) {
        ZGLogDebug(@"invalid action arguments: %@", args);
        return nil;
    }
    ...
    void (^executeBlock)(id, SEL) = ^(id view, SEL command) {
        [NSThread mp_safelyRunOnMainThreadSync:^{
            ...
            NSArray *invocations = [[self class] executeSelector:self.selector
                                                        withArgs:self.args
                                                          onPath:self.path
                                                        fromRoot:[Zhuge sharedUIApplication].keyWindow.rootViewController
                                                          toLeaf:view];
        ...
        if (signature != nil) {
            ...
            if (args.count >= requiredArgs) {
                ...
                @catch (NSException *exception) {
                    ZGLogDebug(@"Exception during invocation: %@", exception);
                }
                [invocations addObject:invocation];
            } else {
                ZGLogDebug(@"Not enough args");
            }
        } else {
            ZGLogDebug(@"No method found for %@", NSStringFromSelector(selector));
        }
    }

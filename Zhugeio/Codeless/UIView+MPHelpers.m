...
- (UIImage *)mp_snapshotImage {
    ...
    @try {
        ...
        ZGLogDebug(@"exception getting snapshot image %@ for view %@", exception, self);
    }
...
- (void)setZhugeioViewId:(id)object
{
    objc_setAssociatedObject(self, @selector(ZhugeioViewId), [object copy], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (NSString *)mp_viewId
{
    return objc_getAssociatedObject(self, @selector(ZhugeioViewId));
}

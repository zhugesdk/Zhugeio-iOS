...
#ifndef ZGGMSM2_CIPHERTEXT_ST_1
#define ZGGMSM2_CIPHERTEXT_ST_1

typedef struct ZGSM2_Ciphertext_st_1 ZGSM2_Ciphertext_1;
DECLARE_ASN1_FUNCTIONS(ZGSM2_Ciphertext_1)

struct ZGSM2_Ciphertext_st_1 {
    BIGNUM *C1x;
    BIGNUM *C1y;
...
ASN1_SEQUENCE(ZGSM2_Ciphertext_1) = {
    ASN1_SIMPLE(ZGSM2_Ciphertext_1, C1x, BIGNUM),
    ASN1_SIMPLE(ZGSM2_Ciphertext_1, C1y, BIGNUM),
    ASN1_SIMPLE(ZGSM2_Ciphertext_1, C3, ASN1_OCTET_STRING),
    ASN1_SIMPLE(ZGSM2_Ciphertext_1, C2, ASN1_OCTET_STRING),
} ASN1_SEQUENCE_END(ZGSM2_Ciphertext_1)

IMPLEMENT_ASN1_FUNCTIONS(ZGSM2_Ciphertext_1)

#endif /* ZGGMSM2_CIPHERTEXT_ST_1 */

// 默认椭圆曲线类型 NID_sm2
static int kZGDefaultEllipticCurveType = NID_sm2;

@implementation ZGGMSm2Utils
...
+ (int)ellipticCurveType {
    return kZGDefaultEllipticCurveType;
}

+ (void)setEllipticCurveType:(int)curveType {
    kZGDefaultEllipticCurveType = curveType;
}
...
+ (NSArray<NSString *> *)createKeyPair{
    ...
    EC_GROUP *group = EC_GROUP_new_by_curve_name(kZGDefaultEllipticCurveType); // 椭圆曲线
    EC_KEY *key = NULL; // 密钥对
    do {
        key = EC_KEY_new();
...
+ (NSData *)asn1EnC1Hex:(NSString *)c1 c3Data:(NSData *)c3 c2Data:(NSData *)c2{
    ...
    // ASN1 编码后存储数据的结构体
    struct ZGSM2_Ciphertext_st_1 ctext_st;
    ctext_st.C2 = NULL;
    ctext_st.C3 = NULL;
    BIGNUM *x1 = NULL;
    BIGNUM *y1 = NULL;
    ...
    do {
        ...
        int asn1_len = i2d_ZGSM2_Ciphertext_1(&ctext_st, &asn1_buf);
...
+ (NSArray<NSData *> *)asn1DeToC1C3C2Data:(NSData *)asn1Data{
    ...
    const EVP_MD *digest = EVP_sm3(); // 摘要算法
    struct ZGSM2_Ciphertext_st_1 *sm2_st = NULL;
    sm2_st = d2i_ZGSM2_Ciphertext_1(NULL, &asn1_ctext, asn1_ctext_len);
    // C1
    ...
    ZGSM2_Ciphertext_1_free(sm2_st);
...
+ (nullable NSString *)signData:(NSData *)plainData priKey:(NSString *)priKey userData:(nullable NSData *)userData{
    ...
    const EVP_MD *digest = EVP_sm3();  // 摘要算法
    EC_GROUP *group = EC_GROUP_new_by_curve_name(kZGDefaultEllipticCurveType);

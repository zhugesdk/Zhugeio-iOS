...
NSString *const MPWebSocketErrorDomain = @"com.Zhugeio.error.WebSocket";

// Returns number of bytes consumed. Returning 0 means you didn't match.
// Sends bytes to callback handler;
typedef size_t (^stream_scanner)(NSData *collected_data);

typedef void (^data_callback)(ZGWebSocket *webSocket,  NSData *data);
...
@interface ZGWebSocket ()  <NSStreamDelegate>
...
@implementation ZGWebSocket {
    ...
    __strong ZGWebSocket *_selfRetain;
...
- (void)_HTTPHeadersDidFinish
{
    ...
    if (responseCode >= 400) {
        ZGLogDebug(@"Request failed with response code %d", responseCode);
...
- (void)_readHTTPHeader
{
    ...
    [self _readUntilHeaderCompleteWithCallback:^(ZGWebSocket *websocket,  NSData *data) {
        ...
        if (CFHTTPMessageIsHeaderComplete(websocket->_receivedHTTPHeaders)) {
            ZGLogDebug(@"Finished reading headers %@", CFBridgingRelease(CFHTTPMessageCopyAllHeaderFields(websocket->_receivedHTTPHeaders)));
...
- (void)didConnect
{
    ...
    if (result != 0) {
        ZGLogDebug(@"Failed to generate random bytes with status: %d", result);
    }
...
- (void)closeWithCode:(NSInteger)code reason:(NSString *)reason
{
    ...
    dispatch_async(_workQueue, ^{
        ...
        ZGLogDebug(@"Closing with code %d reason %@", code, reason);
        ...
        if (reason) {
            ...
            if (!success) {
                ZGLogDebug(@"WPWebSocket: coseWithCode convertion was not successful");
            }
...
- (void)_failWithError:(NSError *)error
{
    dispatch_async(_workQueue, ^{
        if (self.readyState != MPWebSocketStateClosed) {
            ...
            ZGLogDebug(@"Failing with error %@", error.localizedDescription);
...
- (void)handleCloseWithData:(NSData *)data
{
    ...
    ZGLogDebug(@"Received close frame");
...
{
    [self assertOnWorkQueue];
    ZGLogDebug(@"Trying to disconnect");
...
- (void)_handleFrameHeader:(frame_header)frame_header curData:(NSData *)curData
{
    ...
    if (frame_header.payload_length == 0) {
        ...
        [self _addConsumerWithDataLength:(size_t)frame_header.payload_length callback:^(ZGWebSocket *websocket, NSData *newData) {
            if (isControlFrame) {
...
- (void)_readFrameContinue
{
    ...
    [self _addConsumerWithDataLength:2 callback:^(ZGWebSocket *websocket, NSData *data) {
        ...
        if (extra_bytes_needed == 0) {
            ...
            [websocket _addConsumerWithDataLength:extra_bytes_needed callback:^(ZGWebSocket *websocket2, NSData *data2) {
...
//                ZGLogDebug(@"NSStreamEventOpenCompleted %@", aStream);
                if (self.readyState >= MPWebSocketStateClosing) {
                    return;
                }
...
//                ZGLogDebug(@"NSStreamEventErrorOccurred %@ %@", aStream, [[aStream streamError] copy]);
            ...
            case NSStreamEventEndEncountered: {
                [self _pumpScanner];
//                ZGLogDebug(@"NSStreamEventEndEncountered %@", aStream);
...
//                ZGLogDebug(@"NSStreamEventHasBytesAvailable %@", aStream);
...
//                ZGLogDebug(@"NSStreamEventHasSpaceAvailable %@", aStream);
                [self _pumpWriting];
                break;
            }

            default:
                ZGLogDebug(@"(default) %@", aStream);
                break;
        }
    });
}
...
+ (NSRunLoop *)mp_networkRunLoop {
    ...
    dispatch_once(&onceToken, ^{
        ...
        networkThread.name = @"com.Zhugeio.WebSocket.NetworkThread";
        [networkThread start];

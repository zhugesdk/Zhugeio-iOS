...
- (void) processResponse: (DLServerResponse *) response {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.callback) self.callback([self getLatestReferringParams], nil,response.tag);
        double time = CACurrentMediaTime() - self.starttime;
        [DLTime addtimestamp:[self typeofRequest] withtime:time];
    });
}
...
- (BOOL) sendRequest: (DLServerInterface *) serverInterface withcallback: (Servercallback) callback {
    ...
    [post setObject:[DLPreferenceHelper getLinkClickID] forKey:@"click_id"];
    [post setObject:[DLPreferenceHelper getWCookie] forKey:@"wcookie"];
    [post setObject:[DLPreferenceHelper getShortSeg] forKey:@"short_seg"];
    ...
    [post setObject:[NSString stringWithFormat:@"ios%@", ZG_SDK_VERSION] forKey:@"sdk_info"];
